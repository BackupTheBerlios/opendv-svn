...
CDStarClientThread::CDStarClientThread() :
...
void CDStarClientThread::kill()
{
	m_killed = true;
}

void CDStarClientThread::setReader(CWAVFileReader* reader)
{
...
void CDStarClientThread::setCallsign(const wxString& callsign1, const wxString& callsign2)
{
	m_callsign1 = callsign1;
	m_callsign2 = callsign2;
}

void CDStarClientThread::setAudioSoundCard(CSoundCardReaderWriter* soundcard)
{
...
void CDStarClientThread::setRadioSoundCard(CSoundCardReaderWriter* soundcard)
{
	wxASSERT(soundcard != NULL);
...
void CDStarClientThread::setInversion(bool rxInvert, bool txInvert)
{
	m_demodulator.setInvert(rxInvert);
	m_modulator.setInvert(txInvert);
}

void CDStarClientThread::setDVDongle(CDVDongleController* dongle)
{
...
void CDStarClientThread::setController(IExternalController* controller, bool pttEnable, unsigned int txDelay, bool txInvert)
{
...
void CDStarClientThread::setMessage(const wxString& text)
{
	m_message = text;
}

void CDStarClientThread::setRecording(bool record)
{
	m_record = record;
...
void CDStarClientThread::setBleep(bool on)
{
	if (m_dongle != NULL)
		m_dongle->setBleep(on);
}

void CDStarClientThread::setYour(const wxString& your)
{
	m_your = your;
}

void CDStarClientThread::setRpt1(const wxString& rpt1)
{
...
void CDStarClientThread::setRpt2(const wxString& rpt2)
{
...
bool CDStarClientThread::setTransmit(bool transmit)
{
	if (m_stopped)
		return false;
...
void CDStarClientThread::receive()
{
	resetReceiver();
...
void CDStarClientThread::transmit()
{
	m_radioBuffer.clear();
...
void CDStarClientThread::transmitBits()
{
	bool tx = false;
...
void CDStarClientThread::resetReceiver()
{
	setState(RX_STATE_LISTENING);
...
void CDStarClientThread::stateMachine(bool bit)
{
	switch (m_state) {
...
bool CDStarClientThread::processHeader()
{
...
void CDStarClientThread::processFrame(bool sync)
{
...
void CDStarClientThread::processSlowData(const unsigned char* buffer)
{
...
void CDStarClientThread::callback(const wxFloat32* input, wxFloat32* output, unsigned int nSamples, int id)
{
...
void CDStarClientThread::endOfData()
{
	// Tell the GUI
...
unsigned char CDStarClientThread::bitsToByte(const bool* bits) const
{
	wxASSERT(bits != NULL);
...
void CDStarClientThread::byteToBits(unsigned char byte, bool* data) const
{
	wxASSERT(data != NULL);
...
void CDStarClientThread::setState(RX_STATE state)
{
...
void CDStarClientThread::readRadioData(const wxFloat32* input, unsigned int nSamples)
{
	wxASSERT(input != NULL);
