...
BEGIN_EVENT_TABLE(CDVRPTRClientFrame, wxFrame)
	EVT_MENU(Menu_File_Recording, CDVRPTRClientFrame::onRecording)
	EVT_MENU(wxID_EXIT, CDVRPTRClientFrame::onQuit)
	EVT_MENU(Menu_Edit_Preferences, CDVRPTRClientFrame::onPreferences)
	EVT_MENU(wxID_ABOUT, CDVRPTRClientFrame::onAbout)

	EVT_CLOSE(CDVRPTRClientFrame::onClose)

	EVT_TEXT_ENTER(Combo_Your, CDVRPTRClientFrame::onYour)
	EVT_COMBOBOX(Combo_Your, CDVRPTRClientFrame::onYour)

	EVT_TEXT_ENTER(Combo_Rpt1, CDVRPTRClientFrame::onRpt1)
	EVT_COMBOBOX(Combo_Rpt1, CDVRPTRClientFrame::onRpt1)

	EVT_TEXT_ENTER(Combo_Rpt2, CDVRPTRClientFrame::onRpt2)
	EVT_COMBOBOX(Combo_Rpt2, CDVRPTRClientFrame::onRpt2)

	EVT_TOGGLEBUTTON(Button_Transmit, CDVRPTRClientFrame::onTransmit)

	EVT_BUTTON(Button_OneTouchReply, CDVRPTRClientFrame::onOneTouchReply)

	EVT_CUSTOM(MESSAGE_EVENT, wxID_ANY, CDVRPTRClientFrame::onMessage)
	EVT_CUSTOM(HEADER_EVENT,  wxID_ANY, CDVRPTRClientFrame::onHeader)
	EVT_CUSTOM(ERROR_EVENT,   wxID_ANY, CDVRPTRClientFrame::onError)
	EVT_CUSTOM(BUSY_EVENT,    wxID_ANY, CDVRPTRClientFrame::onBusy)
END_EVENT_TABLE()

CDVRPTRClientFrame::CDVRPTRClientFrame(const wxString& title) :
...
CDVRPTRClientFrame::~CDVRPTRClientFrame()
{
}

wxMenuBar* CDVRPTRClientFrame::createMenuBar()
{
	...
	helpMenu->Append(wxID_ABOUT, _("About DVRPTR Client"));
...
void CDVRPTRClientFrame::showMessage(CMessageData* message)
{
...
void CDVRPTRClientFrame::showHeader(CHeaderData* header)
{
...
void CDVRPTRClientFrame::showBusy(CBusyData* busy)
{
	wxASSERT(busy != NULL);
...
void CDVRPTRClientFrame::error(const wxString& error)
{
...
void CDVRPTRClientFrame::onRecording(wxCommandEvent& event)
{
...
void CDVRPTRClientFrame::onQuit(wxCommandEvent& event)
{
	Close(false);
}

void CDVRPTRClientFrame::onClose(wxCloseEvent& event)
{
	if (!event.CanVeto()) {
		saveCalls();
		Destroy();
	...
	int reply = ::wxMessageBox(_("Do you want to exit DVRPTR Client"), _("Exit DVRPTR Client"), wxOK | wxCANCEL | wxICON_QUESTION);
	switch (reply) {
		case wxOK:
			saveCalls();
			Destroy();
...
void CDVRPTRClientFrame::onPreferences(wxCommandEvent& event)
{
	...
	bool     newChannel       = dialog.getChannel();
	unsigned int newModLevel  = dialog.getModLevel();
	...
	if (changed) {
		wxMessageDialog dialog(this, _("The changes made will not take effect\nuntil the application is restarted"), _("DVRPTR Client Information"), wxICON_INFORMATION);
		dialog.ShowModal();
	}
}

void CDVRPTRClientFrame::onAbout(wxCommandEvent& event)
{
	wxAboutDialogInfo info;
	info.AddDeveloper(wxT("Jonathan Naylor, G4KLX"));
	info.SetName(APPLICATION_NAME);
	info.SetVersion(VERSION);
	info.SetDescription(_("This program allows a computer with a DV Dongle\na DV-RPTR modem and a 9600Bd capable radio to work D-Star stations\nwith or without a repeater."));

	::wxAboutBox(info);
}

void CDVRPTRClientFrame::onYour(wxCommandEvent& event)
{
...
bool CDVRPTRClientFrame::processYour(wxString& your)
{
	your.MakeUpper();
...
void CDVRPTRClientFrame::onRpt1(wxCommandEvent& event)
{
...
bool CDVRPTRClientFrame::processRpt1(wxString& rpt1)
{
...
void CDVRPTRClientFrame::onRpt2(wxCommandEvent& event)
{
...
bool CDVRPTRClientFrame::processRpt2(wxString& rpt2)
{
...
void CDVRPTRClientFrame::onTransmit(wxCommandEvent& event)
{
...
void CDVRPTRClientFrame::onOneTouchReply(wxCommandEvent& event)
{
...
void CDVRPTRClientFrame::onMessage(wxEvent& event)
{
...
void CDVRPTRClientFrame::onHeader(wxEvent& event)
{
...
void CDVRPTRClientFrame::onBusy(wxEvent& event)
{
...
void CDVRPTRClientFrame::onError(wxEvent& event)
{
	CErrorEvent& errEvent = dynamic_cast<CErrorEvent&>(event);

	wxString text = errEvent.getText();

	wxMessageDialog dialog(this, text, _("DVRPTR Client Error"), wxICON_ERROR);
	dialog.ShowModal();
}

void CDVRPTRClientFrame::saveCalls() const
{
