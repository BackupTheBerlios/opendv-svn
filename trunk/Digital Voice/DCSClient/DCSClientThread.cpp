...
CDCSClientThread::CDCSClientThread() :
...
void CDCSClientThread::kill()
{
	m_killed = true;
}

void CDCSClientThread::setCallsign(const wxString& callsign)
{
	m_callsign = callsign;
...
void CDCSClientThread::setSoundCard(CSoundCardReaderWriter* soundcard)
{
...
void CDCSClientThread::setDVDongle(CDVDongleController* dongle)
{
...
void CDCSClientThread::setBleep(bool on)
{
	if (m_dongle != NULL)
		m_dongle->setBleep(on);
}

void CDCSClientThread::setController(IExternalController* controller, unsigned int audioDelay, bool pttInvert, bool squelchInvert)
{
...
bool CDCSClientThread::connect(bool connect, const wxString& reflector, const wxString& address)
{
	if (m_stopped)
		return false;
...
bool CDCSClientThread::transmit(bool transmit)
{
	if (m_stopped)
		return false;
...
bool CDCSClientThread::isConnected()
{
	return m_protocol.isConnected();
}

void CDCSClientThread::receive()
{
	...
	while (m_transmit == CLIENT_RECEIVE && !m_killed) {
		...
		switch (type) {
			...
			case NETWORK_DATA: {
					...
					unsigned int length;
					bool start, sync, end;
					length = m_protocol.readData(buffer, DV_FRAME_LENGTH_BYTES, start, sync, end);
					if (length > 0U)
						processFrame(buffer, start, sync, end);
				}
				break;
		}
...
void CDCSClientThread::transmit()
{
	m_encodeData.clear();
...
void CDCSClientThread::processFrame(const unsigned char* buffer, bool start, bool sync, bool end)
{
	if (start) {
		CHeaderData* header = m_protocol.getHeader();
...
void CDCSClientThread::processSlowData(const unsigned char* buffer)
{
...
void CDCSClientThread::callback(const wxFloat32* input, wxFloat32* output, unsigned int nSamples, int id)
{
...
void CDCSClientThread::resetReceiver()
{
	// Tell the GUI
