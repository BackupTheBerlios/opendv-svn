...
bool CDCSClientApp::OnInit()
{
	...
	m_frame = new CDCSClientFrame(APPLICATION_NAME + wxT(" - ") + VERSION);
	m_frame->Show();
...
void CDCSClientApp::OnAssertFailure(const wxChar* file, int line, const wxChar* func, const wxChar* cond, const wxChar* msg)
{
	wxLogFatalError(wxT("Assertion failed on line %d in file %s and function %s: %s %s"), line, file, func, cond, msg);
}
#endif

void CDCSClientApp::showMessage(CMessageData* message) const
{
...
void CDCSClientApp::showHeader(CHeaderData* header) const
{
	m_frame->showHeader(header);
}

void CDCSClientApp::error(const wxString& text) const
{
	m_frame->error(text);
}

bool CDCSClientApp::connect(bool connect, const wxString& reflector, const wxString& address)
{
	return m_thread->connect(connect, reflector, address);
}

bool CDCSClientApp::transmit(bool transmit)
{
	return m_thread->transmit(transmit);
}

void CDCSClientApp::setRecording(bool recording)
{
	m_thread->setRecording(recording);
}

bool CDCSClientApp::isConnected()
{
	return m_thread->isConnected();
}

void CDCSClientApp::getCallsign(wxString& callsign) const
{
...
void CDCSClientApp::getMessage(wxString& message) const
{
	wxConfigBase* profile = new wxConfig(APPLICATION_NAME);
	wxASSERT(profile != NULL);
...
void CDCSClientApp::setMessage(const wxString& message) const
{
	wxConfigBase* profile = new wxConfig(APPLICATION_NAME);
	wxASSERT(profile != NULL);
...
void CDCSClientApp::getSoundcard(wxString& readDevice, wxString& writeDevice) const
{
	wxConfigBase* profile = new wxConfig(APPLICATION_NAME);
	wxASSERT(profile != NULL);
...
void CDCSClientApp::setSoundcard(const wxString& readDevice, const wxString& writeDevice) const
{
	wxConfigBase* profile = new wxConfig(APPLICATION_NAME);
	wxASSERT(profile != NULL);
...
void CDCSClientApp::getDVDongle(wxString& device) const
{
	wxConfigBase* profile = new wxConfig(APPLICATION_NAME);
	wxASSERT(profile != NULL);
...
void CDCSClientApp::setDVDongle(const wxString& device) const
{
	wxConfigBase* profile = new wxConfig(APPLICATION_NAME);
	wxASSERT(profile != NULL);
...
void CDCSClientApp::getLastReflector(wxString& reflector) const
{
	wxConfigBase* profile = new wxConfig(APPLICATION_NAME);
	wxASSERT(profile != NULL);
...
void CDCSClientApp::setLastReflector(const wxString& reflector) const
{
	wxConfigBase* profile = new wxConfig(APPLICATION_NAME);
	wxASSERT(profile != NULL);
...
void CDCSClientApp::getBleep(bool& on) const
{
	wxConfigBase* profile = new wxConfig(APPLICATION_NAME);
	wxASSERT(profile != NULL);
...
void CDCSClientApp::setBleep(bool on) const
{
	wxConfigBase* profile = new wxConfig(APPLICATION_NAME);
	wxASSERT(profile != NULL);
...
void CDCSClientApp::getController(wxString& type, unsigned int& audioDelay, bool& squelchInvert, bool& pttInvert) const
{
	wxConfigBase* profile = new wxConfig(APPLICATION_NAME);
	wxASSERT(profile != NULL);
...
void CDCSClientApp::getStartup(wxString& reflector) const
{
	wxConfigBase* profile = new wxConfig(APPLICATION_NAME);
	wxASSERT(profile != NULL);
	...
	wxLogInfo(wxT("Startup Reflector set to: %s"), reflector.c_str());

	delete profile;
}

void CDCSClientApp::setStartup(const wxString& reflector) const
{
	wxConfigBase* profile = new wxConfig(APPLICATION_NAME);
	wxASSERT(profile != NULL);
