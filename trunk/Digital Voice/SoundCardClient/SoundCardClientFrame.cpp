...
BEGIN_EVENT_TABLE(CSoundCardClientFrame, wxFrame)
	EVT_MENU(wxID_OPEN, CSoundCardClientFrame::onOpen)
	EVT_MENU(Menu_File_Recording, CSoundCardClientFrame::onRecording)
	EVT_MENU(wxID_EXIT, CSoundCardClientFrame::onQuit)
	EVT_MENU(Menu_Edit_Preferences, CSoundCardClientFrame::onPreferences)
	EVT_MENU(wxID_ABOUT, CSoundCardClientFrame::onAbout)

	EVT_CLOSE(CSoundCardClientFrame::onClose)

	EVT_TEXT_ENTER(Combo_Your, CSoundCardClientFrame::onYour)
	EVT_COMBOBOX(Combo_Your, CSoundCardClientFrame::onYour)

	EVT_TEXT_ENTER(Combo_Rpt1, CSoundCardClientFrame::onRpt1)
	EVT_COMBOBOX(Combo_Rpt1, CSoundCardClientFrame::onRpt1)

	EVT_TEXT_ENTER(Combo_Rpt2, CSoundCardClientFrame::onRpt2)
	EVT_COMBOBOX(Combo_Rpt2, CSoundCardClientFrame::onRpt2)

	EVT_TOGGLEBUTTON(Button_Transmit, CSoundCardClientFrame::onTransmit)

	EVT_BUTTON(Button_OneTouchReply, CSoundCardClientFrame::onOneTouchReply)

	EVT_CUSTOM(MESSAGE_EVENT, wxID_ANY, CSoundCardClientFrame::onMessage)
	EVT_CUSTOM(HEADER_EVENT,  wxID_ANY, CSoundCardClientFrame::onHeader)
	EVT_CUSTOM(ERROR_EVENT,   wxID_ANY, CSoundCardClientFrame::onError)
	EVT_CUSTOM(BUSY_EVENT,    wxID_ANY, CSoundCardClientFrame::onBusy)
END_EVENT_TABLE()

CSoundCardClientFrame::CSoundCardClientFrame(const wxString& title) :
...
m_heard(NULL)
{
	SetIcon(wxICON(SoundCardClient));
...
CSoundCardClientFrame::~CSoundCardClientFrame()
{
}

wxMenuBar* CSoundCardClientFrame::createMenuBar()
{
	...
	helpMenu->Append(wxID_ABOUT, _("About D-Star Client"));
...
void CSoundCardClientFrame::showMessage(CMessageData* message)
{
...
void CSoundCardClientFrame::showHeader(CHeaderData* header)
{
...
void CSoundCardClientFrame::showBusy(CBusyData* busy)
{
	wxASSERT(busy != NULL);
...
void CSoundCardClientFrame::error(const wxString& error)
{
...
void CSoundCardClientFrame::onOpen(wxCommandEvent& event)
{
...
void CSoundCardClientFrame::onRecording(wxCommandEvent& event)
{
...
void CSoundCardClientFrame::onQuit(wxCommandEvent& event)
{
	Close(false);
}

void CSoundCardClientFrame::onClose(wxCloseEvent& event)
{
	if (!event.CanVeto()) {
		saveCalls();
		Destroy();
	...
	int reply = ::wxMessageBox(_("Do you want to exit Sound Card Client"), _("Exit Sound Card Client"), wxOK | wxCANCEL | wxICON_QUESTION);
	switch (reply) {
		case wxOK:
			saveCalls();
			Destroy();
...
void CSoundCardClientFrame::onPreferences(wxCommandEvent& event)
{
	wxString oldCallsign1, oldCallsign2;
	::wxGetApp().getCallsign(oldCallsign1, oldCallsign2);

	wxString oldAudioReadDevice, oldAudioWriteDevice;
	::wxGetApp().getAudioSoundcard(oldAudioReadDevice, oldAudioWriteDevice);

	wxString oldRadioReadDevice, oldRadioWriteDevice;
	::wxGetApp().getRadioSoundcard(oldRadioReadDevice, oldRadioWriteDevice);

	bool oldRXGMSKInvert, oldTXGMSKInvert;
	::wxGetApp().getInversion(oldRXGMSKInvert, oldTXGMSKInvert);

	wxString oldIfaceType;
	unsigned int oldTXDelay;
	bool oldPTTEnable, oldTXInvert;
	::wxGetApp().getController(oldIfaceType, oldPTTEnable, oldTXDelay, oldTXInvert);
	...
	CSoundCardClientPreferences dialog(this, -1, oldCallsign1, oldCallsign2, oldAudioReadDevice, oldAudioWriteDevice,
								   oldRadioReadDevice, oldRadioWriteDevice, oldRXGMSKInvert, oldTXGMSKInvert,
								   oldIfaceType, oldPTTEnable, oldTXDelay, oldTXInvert, oldDVDDevice, oldMessage,
								   oldBleep);
	if (dialog.ShowModal() != wxID_OK)
		return;
	...
	wxString newAudioReadDevice  = dialog.getAudioSoundcardReadDevice();
	wxString newAudioWriteDevice = dialog.getAudioSoundcardWriteDevice();
	wxString newRadioReadDevice  = dialog.getRadioSoundcardReadDevice();
	wxString newRadioWriteDevice = dialog.getRadioSoundcardWriteDevice();
	bool newRXGMSKInvert         = dialog.getRXGMSKInvert();
	bool newTXGMSKInvert         = dialog.getTXGMSKInvert();
	wxString newIfaceType        = dialog.getIfaceType();
	bool newPTTEnable            = dialog.getPTTEnable();
	unsigned int newTXDelay      = dialog.getTXDelay();
	...
	if (!newAudioReadDevice.IsSameAs(oldAudioReadDevice) || !newAudioWriteDevice.IsSameAs(oldAudioWriteDevice)) {
		::wxGetApp().setAudioSoundcard(newAudioReadDevice, newAudioWriteDevice);
		changed = true;
	}

	if (!newRadioReadDevice.IsSameAs(oldRadioReadDevice) || !newRadioWriteDevice.IsSameAs(oldRadioWriteDevice)) {
		::wxGetApp().setRadioSoundcard(newRadioReadDevice, newRadioWriteDevice);
		changed = true;
	}

	if (newRXGMSKInvert != oldRXGMSKInvert || newTXGMSKInvert != oldTXGMSKInvert)
		::wxGetApp().setInversion(newRXGMSKInvert, newTXGMSKInvert);

	if (!newIfaceType.IsSameAs(oldIfaceType) || newPTTEnable != oldPTTEnable || newTXDelay != oldTXDelay || newTXInvert != oldTXInvert) {
		::wxGetApp().setController(newIfaceType, newPTTEnable, newTXDelay, newTXInvert);
		changed = true;
	}
...
void CSoundCardClientFrame::onAbout(wxCommandEvent& event)
{
	wxAboutDialogInfo info;
	info.AddDeveloper(wxT("Jonathan Naylor, G4KLX"));
	info.SetName(APPLICATION_NAME);
	info.SetVersion(VERSION);
	info.SetDescription(_("This program allows a computer with a DV Dongle\nand a 9600Bd capable radio to work D-Star stations\nwith or without a repeater."));

	::wxAboutBox(info);
}

void CSoundCardClientFrame::onYour(wxCommandEvent& event)
{
...
bool CSoundCardClientFrame::processYour(wxString& your)
{
	your.MakeUpper();
...
void CSoundCardClientFrame::onRpt1(wxCommandEvent& event)
{
...
bool CSoundCardClientFrame::processRpt1(wxString& rpt1)
{
...
void CSoundCardClientFrame::onRpt2(wxCommandEvent& event)
{
...
bool CSoundCardClientFrame::processRpt2(wxString& rpt2)
{
...
void CSoundCardClientFrame::onTransmit(wxCommandEvent& event)
{
...
void CSoundCardClientFrame::onOneTouchReply(wxCommandEvent& event)
{
...
void CSoundCardClientFrame::onMessage(wxEvent& event)
{
...
void CSoundCardClientFrame::onHeader(wxEvent& event)
{
...
void CSoundCardClientFrame::onBusy(wxEvent& event)
{
...
void CSoundCardClientFrame::onError(wxEvent& event)
{
	CErrorEvent& errEvent = dynamic_cast<CErrorEvent&>(event);

	wxString text = errEvent.getText();

	wxMessageDialog dialog(this, text, _("D-Star Client Error"), wxICON_ERROR);
	dialog.ShowModal();
}

void CSoundCardClientFrame::saveCalls() const
{
