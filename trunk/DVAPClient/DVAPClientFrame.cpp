...
BEGIN_EVENT_TABLE(CDVAPClientFrame, wxFrame)
	EVT_MENU(Menu_File_Recording, CDVAPClientFrame::onRecording)
	EVT_MENU(wxID_EXIT, CDVAPClientFrame::onQuit)
	EVT_MENU(Menu_Edit_Preferences, CDVAPClientFrame::onPreferences)
	EVT_MENU(wxID_ABOUT, CDVAPClientFrame::onAbout)

	EVT_CLOSE(CDVAPClientFrame::onClose)

	EVT_TEXT_ENTER(Combo_Your, CDVAPClientFrame::onYour)
	EVT_COMBOBOX(Combo_Your, CDVAPClientFrame::onYour)

	EVT_TEXT_ENTER(Combo_Rpt1, CDVAPClientFrame::onRpt1)
	EVT_COMBOBOX(Combo_Rpt1, CDVAPClientFrame::onRpt1)

	EVT_TEXT_ENTER(Combo_Rpt2, CDVAPClientFrame::onRpt2)
	EVT_COMBOBOX(Combo_Rpt2, CDVAPClientFrame::onRpt2)

	EVT_TOGGLEBUTTON(Button_Transmit, CDVAPClientFrame::onTransmit)

	EVT_BUTTON(Button_OneTouchReply, CDVAPClientFrame::onOneTouchReply)

	EVT_CUSTOM(MESSAGE_EVENT, wxID_ANY, CDVAPClientFrame::onMessage)
	EVT_CUSTOM(HEADER_EVENT,  wxID_ANY, CDVAPClientFrame::onHeader)
	EVT_CUSTOM(ERROR_EVENT,   wxID_ANY, CDVAPClientFrame::onError)
	EVT_CUSTOM(BUSY_EVENT,    wxID_ANY, CDVAPClientFrame::onBusy)
END_EVENT_TABLE()

CDVAPClientFrame::CDVAPClientFrame(const wxString& title) :
...
CDVAPClientFrame::~CDVAPClientFrame()
{
}

wxMenuBar* CDVAPClientFrame::createMenuBar()
{
	...
	helpMenu->Append(wxID_ABOUT, _("About DVAP Client"));
...
void CDVAPClientFrame::showMessage(CMessageData* message)
{
...
void CDVAPClientFrame::showHeader(CHeaderData* header)
{
...
void CDVAPClientFrame::showBusy(CBusyData* busy)
{
	wxASSERT(busy != NULL);
...
void CDVAPClientFrame::error(const wxString& error)
{
...
void CDVAPClientFrame::onRecording(wxCommandEvent& event)
{
...
void CDVAPClientFrame::onQuit(wxCommandEvent& event)
{
	Close(false);
}

void CDVAPClientFrame::onClose(wxCloseEvent& event)
{
	if (!event.CanVeto()) {
		saveCalls();
		Destroy();
	...
	int reply = ::wxMessageBox(_("Do you want to exit DVAP Client"), _("Exit DVAP Client"), wxOK | wxCANCEL | wxICON_QUESTION);
	switch (reply) {
		case wxOK:
			saveCalls();
			Destroy();
...
void CDVAPClientFrame::onPreferences(wxCommandEvent& event)
{
	...
	if (changed) {
		wxMessageDialog dialog(this, _("The changes made will not take effect\nuntil the application is restarted"), _("DVAP Client Information"), wxICON_INFORMATION);
		dialog.ShowModal();
	}
}

void CDVAPClientFrame::onAbout(wxCommandEvent& event)
{
	wxAboutDialogInfo info;
	info.AddDeveloper(wxT("Jonathan Naylor, G4KLX"));
	info.SetName(APPLICATION_NAME);
	info.SetVersion(VERSION);
	info.SetDescription(_("This program allows a computer with a DV Dongle\na DVAP to work D-Star stations\nwith or without a repeater."));

	::wxAboutBox(info);
}

void CDVAPClientFrame::onYour(wxCommandEvent& event)
{
...
bool CDVAPClientFrame::processYour(wxString& your)
{
	your.MakeUpper();
...
void CDVAPClientFrame::onRpt1(wxCommandEvent& event)
{
...
bool CDVAPClientFrame::processRpt1(wxString& rpt1)
{
...
void CDVAPClientFrame::onRpt2(wxCommandEvent& event)
{
...
bool CDVAPClientFrame::processRpt2(wxString& rpt2)
{
...
void CDVAPClientFrame::onTransmit(wxCommandEvent& event)
{
...
void CDVAPClientFrame::onOneTouchReply(wxCommandEvent& event)
{
...
void CDVAPClientFrame::onMessage(wxEvent& event)
{
...
void CDVAPClientFrame::onHeader(wxEvent& event)
{
...
void CDVAPClientFrame::onBusy(wxEvent& event)
{
...
void CDVAPClientFrame::onError(wxEvent& event)
{
	CErrorEvent& errEvent = dynamic_cast<CErrorEvent&>(event);

	wxString text = errEvent.getText();

	wxMessageDialog dialog(this, text, _("DVAP Client Error"), wxICON_ERROR);
	dialog.ShowModal();
}

void CDVAPClientFrame::saveCalls() const
{
