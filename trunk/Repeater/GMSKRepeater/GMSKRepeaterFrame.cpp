...
BEGIN_EVENT_TABLE(CGMSKRepeaterFrame, wxFrame)
	EVT_MENU(Menu_File_Logging,     CGMSKRepeaterFrame::onLogging)
	EVT_MENU(wxID_EXIT,             CGMSKRepeaterFrame::onQuit)
	EVT_MENU(Menu_View_Updates,     CGMSKRepeaterFrame::onUpdates)
	EVT_MENU(Menu_Edit_Preferences, CGMSKRepeaterFrame::onPreferences)
	EVT_MENU(wxID_ABOUT,            CGMSKRepeaterFrame::onAbout)

	EVT_MENU(Menu_Action_Startup,   CGMSKRepeaterFrame::onActions)
	EVT_MENU(Menu_Action_Shutdown,  CGMSKRepeaterFrame::onActions)
	EVT_MENU(Menu_Action_Command1,  CGMSKRepeaterFrame::onActions)
	EVT_MENU(Menu_Action_Command2,  CGMSKRepeaterFrame::onActions)
	EVT_MENU(Menu_Action_Command3,  CGMSKRepeaterFrame::onActions)
	EVT_MENU(Menu_Action_Command4,  CGMSKRepeaterFrame::onActions)

	EVT_MENU(Menu_Output_1,         CGMSKRepeaterFrame::onOutputs)
	EVT_MENU(Menu_Output_2,         CGMSKRepeaterFrame::onOutputs)
	EVT_MENU(Menu_Output_3,         CGMSKRepeaterFrame::onOutputs)
	EVT_MENU(Menu_Output_4,         CGMSKRepeaterFrame::onOutputs)

	EVT_CLOSE(CGMSKRepeaterFrame::onClose)

	EVT_TIMER(-1, CGMSKRepeaterFrame::onTimer)

	EVT_CUSTOM(LOG_EVENT, wxID_ANY, CGMSKRepeaterFrame::onLog)
END_EVENT_TABLE()
...
void CGMSKRepeaterFrame::onLogging(wxCommandEvent& event)
{
	::wxGetApp().setLogging(event.IsChecked());
}

void CGMSKRepeaterFrame::onQuit(wxCommandEvent& event)
{
	Close(false);
}

void CGMSKRepeaterFrame::onClose(wxCloseEvent& event)
{
	int x, y;
	GetPosition(&x, &y);
	if (x >= 0 && y >= 0) {
...
void CGMSKRepeaterFrame::onActions(wxCommandEvent& event)
{
	int id = event.GetId();
...
void CGMSKRepeaterFrame::onOutputs(wxCommandEvent& event)
{
...
void CGMSKRepeaterFrame::onUpdates(wxCommandEvent& event)
{
	m_updates = event.IsChecked();
}

void CGMSKRepeaterFrame::onAbout(wxCommandEvent& event)
{
	...
	info.SetDescription(_("This program allows a computer with a GMSK modem\nand 9600Bd capable radios to act as a D-Star repeater."));
...
void CGMSKRepeaterFrame::onTimer(wxTimerEvent& event)
{
	if (!m_updates)
		return;

	CGMSKRepeaterStatusData* status = ::wxGetApp().getStatus();
	if (status == NULL)
		return;
...
void CGMSKRepeaterFrame::onLog(wxEvent& event)
{
...
void CGMSKRepeaterFrame::showLog(const wxString& text)
{
	if (!m_updates)
		return;
...
void CGMSKRepeaterFrame::setOutputs(bool out1, bool out2, bool out3, bool out4)
{
