...
CDVDongleController::CDVDongleController(CSerialDataController* controller) :
...
bool CDVDongleController::open()
{
	...
	m_controller->write(DVD_REQ_NAME, DVD_REQ_NAME_LEN);
...
{
	m_controller->write(DVD_REQ_START, DVD_REQ_START_LEN);
...
{
	m_controller->write(DVD_REQ_STOP, DVD_REQ_STOP_LEN);
...
RESP_TYPE CDVDongleController::getResponse(unsigned char* buffer, unsigned int length)
{
...
RESP_TYPE CDVDongleController::processResponse(unsigned char* buffer, unsigned int length)
{
	wxASSERT(buffer != NULL);
	wxASSERT(length >= DVD_AUDIO_LENGTH_BYTES);
