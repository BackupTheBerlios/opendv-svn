...
void CDVRPTRRepeaterTRXThread::setModem(IDVRPTRController* controller)
{
...
void CDVRPTRRepeaterTRXThread::setTimes(unsigned int timeout, unsigned int ackTime)
{
	m_timeoutTimer.setTimeout(timeout);
	m_ackTimer.setTimeout(0U, ackTime);
}

void CDVRPTRRepeaterTRXThread::setBeacon(unsigned int time, const wxString& text, bool voice, TEXT_LANG language)
{
...
void CDVRPTRRepeaterTRXThread::setOutputs(bool out1, bool out2, bool out3, bool out4)
{
...
void CDVRPTRRepeaterTRXThread::setLogging(bool logging, const wxString& dir)
{
...
void CDVRPTRRepeaterTRXThread::receiveRadio()
{
	for (;;) {
...
void CDVRPTRRepeaterTRXThread::receiveHeader(unsigned char* data, unsigned int length)
{
...
void CDVRPTRRepeaterTRXThread::receiveSlowData(unsigned char* data, unsigned int length)
{
	unsigned int errs;
...
void CDVRPTRRepeaterTRXThread::receiveRadioData(unsigned char* data, unsigned int length)
{
	unsigned int errs;
	errs  = countBits(data[VOICE_FRAME_LENGTH_BYTES + 0U] ^ DATA_SYNC_BYTES[0U]);
	errs += countBits(data[VOICE_FRAME_LENGTH_BYTES + 1U] ^ DATA_SYNC_BYTES[1U]);
...
void CDVRPTRRepeaterTRXThread::receiveNetwork()
{
	if (m_protocolHandler == NULL)
		return;
...
void CDVRPTRRepeaterTRXThread::transmitLocalHeader(CHeaderData* header)
{
...
void CDVRPTRRepeaterTRXThread::transmitBeaconHeader()
{
	CHeaderData* header = new CHeaderData(m_rptCallsign, wxT("RPTR"), wxT("CQCQCQ  "), m_dmyCallsign, m_rptCallsign);
	transmitLocalHeader(header);
}

void CDVRPTRRepeaterTRXThread::transmitBeaconData(const unsigned char* data, unsigned int length, bool end)
{
...
void CDVRPTRRepeaterTRXThread::transmitNetworkHeader(CHeaderData* header)
{
	wxLogMessage(wxT("Transmitting to - My: %s/%s  Your: %s  Rpt1: %s  Rpt2: %s  Flags: %02X %02X %02X"), header->getMyCall1().c_str(), header->getMyCall2().c_str(), header->getYourCall().c_str(), header->getRptCall1().c_str(), header->getRptCall2().c_str(), header->getFlag1(), header->getFlag2(), header->getFlag3());
...
void CDVRPTRRepeaterTRXThread::transmitStatus()
{
	CHeaderData* header = new CHeaderData(m_rptCallsign, wxT("    "), m_rxHeader->getMyCall1(), m_dmyCallsign, m_rptCallsign, RELAY_UNAVAILABLE);
	transmitLocalHeader(header);
...
void CDVRPTRRepeaterTRXThread::transmitErrorStatus()
{
...
void CDVRPTRRepeaterTRXThread::transmitUserStatus(unsigned int n)
{
	CSlowDataEncoder* encoder = NULL;
	CHeaderData* header = NULL;
	switch (n) {
		case 0U:
			header = new CHeaderData(m_rptCallsign, wxT("    "), wxT("STATUS 1"), m_dmyCallsign, m_rptCallsign);
			encoder = &m_status1Encoder;
			break;
		case 1U:
			header = new CHeaderData(m_rptCallsign, wxT("    "), wxT("STATUS 2"), m_dmyCallsign, m_rptCallsign);
			encoder = &m_status2Encoder;
			break;
		case 2U:
			header = new CHeaderData(m_rptCallsign, wxT("    "), wxT("STATUS 3"), m_dmyCallsign, m_rptCallsign);
			encoder = &m_status3Encoder;
			break;
		case 3U:
			header = new CHeaderData(m_rptCallsign, wxT("    "), wxT("STATUS 4"), m_dmyCallsign, m_rptCallsign);
			encoder = &m_status4Encoder;
			break;
		case 4U:
			header = new CHeaderData(m_rptCallsign, wxT("    "), wxT("STATUS 5"), m_dmyCallsign, m_rptCallsign);
...
void CDVRPTRRepeaterTRXThread::transmitLocalHeader()
{
...
void CDVRPTRRepeaterTRXThread::transmitLocalData()
{
...
void CDVRPTRRepeaterTRXThread::transmitRadioData()
{
	bool space = m_dvrptr->hasSpace();
	if (!space)
		return;
...
void CDVRPTRRepeaterTRXThread::transmitNetworkData()
{
	bool space = m_dvrptr->hasSpace();
	if (!space)
		return;
...
void CDVRPTRRepeaterTRXThread::repeaterStateMachine()
{
	switch (m_rptState) {
		case DSRS_VALID:
...
void CDVRPTRRepeaterTRXThread::setRadioState(DSTAR_RX_STATE state)
{
...
bool CDVRPTRRepeaterTRXThread::setRepeaterState(DSTAR_RPT_STATE state)
{
...
bool CDVRPTRRepeaterTRXThread::processRadioHeader(CHeaderData* header)
{
	...
	if (m_rptState == DSRS_VALID) {
		...
		if (m_protocolHandler != NULL && !m_rxHeader->getRptCall2().IsSameAs(wxT("        ")) && !m_rxHeader->getRptCall2().IsSameAs(m_rptCallsign)) {
			...
			if (m_rxHeader->getRptCall2().IsSameAs(m_dmyCallsign)) {
...
void CDVRPTRRepeaterTRXThread::processNetworkHeader(CHeaderData* header)
{
	wxASSERT(header != NULL);
	...
	if (m_rptState == DSRS_NETWORK) {
		...
		if (m_mode == MODE_GATEWAY) {
			...
			header->setRptCall2(m_dmyCallsign);
...
void CDVRPTRRepeaterTRXThread::processRadioFrame(unsigned char* data, FRAME_TYPE type)
{
	m_ambeFrames++;
...
unsigned int CDVRPTRRepeaterTRXThread::processNetworkFrame(unsigned char* data, unsigned int length, unsigned char seqNo)
{
	wxASSERT(data != NULL);
	wxASSERT(length > 0U);
...
void CDVRPTRRepeaterTRXThread::endOfRadioData()
{
	switch (m_rptState) {
		case DSRS_VALID:
...
void CDVRPTRRepeaterTRXThread::endOfNetworkData()
{
	if (m_watchdogTimer.hasExpired()) {
		m_packetCount   -= 77U;
		m_packetSilence -= 77U;
...
CDVRPTRRepeaterStatusData* CDVRPTRRepeaterTRXThread::getStatus()
{
...
void CDVRPTRRepeaterTRXThread::clock(unsigned int ms)
{
...
void CDVRPTRRepeaterTRXThread::startup()
{
	m_shutdown = false;
}

void CDVRPTRRepeaterTRXThread::command1()
{
	if (!m_controlCommand1Line.IsEmpty())
		::wxShell(m_controlCommand1Line);
}

void CDVRPTRRepeaterTRXThread::command2()
{
	if (!m_controlCommand2Line.IsEmpty())
		::wxShell(m_controlCommand2Line);
}

void CDVRPTRRepeaterTRXThread::command3()
{
	if (!m_controlCommand3Line.IsEmpty())
		::wxShell(m_controlCommand3Line);
}

void CDVRPTRRepeaterTRXThread::command4()
{
	if (!m_controlCommand4Line.IsEmpty())
		::wxShell(m_controlCommand4Line);
}

bool CDVRPTRRepeaterTRXThread::checkControl(const CHeaderData& header)
{
	if (!m_controlEnabled)
		return false;
...
unsigned int CDVRPTRRepeaterTRXThread::countBits(unsigned char byte)
{
...
void CDVRPTRRepeaterTRXThread::blankDTMF(unsigned char* data)
{
	wxASSERT(data != NULL);
