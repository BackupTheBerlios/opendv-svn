...
void CDStarRepeaterTXRXThread::start()
{
	Create();
...
void CDStarRepeaterTXRXThread::wait()
{
	Wait();
}

void CDStarRepeaterTXRXThread::setReader(CWAVFileReader* reader)
{
...
void CDStarRepeaterTXRXThread::setCallsign(const wxString& callsign, const wxString& gateway, DSTAR_MODE mode, ACK_TYPE ack, bool restriction, bool rpt1Validation)
{
...
void CDStarRepeaterTXRXThread::setSoundCard(CSoundCardReaderWriter* soundcard, wxFloat32 rxLevel, wxFloat32 txLevel, SQUELCH_MODE squelchMode, wxFloat32 squelchLevel, bool rxInvert, bool txInvert)
{
...
void CDStarRepeaterTXRXThread::setTimes(unsigned int timeout, unsigned int ackTime, unsigned int hangTime)
{
...
void CDStarRepeaterTXRXThread::setController(CExternalController* controller, int pttDelay)
{
...
void CDStarRepeaterTXRXThread::setActiveHang(unsigned int time)
{
...
void CDStarRepeaterTXRXThread::setRXLevel(wxFloat32 level)
{
	m_rxLevel = level;
}

void CDStarRepeaterTXRXThread::setTXLevel(wxFloat32 level)
{
	m_txLevel = level;
}

void CDStarRepeaterTXRXThread::setSquelchLevel(wxFloat32 level)
{
...
void CDStarRepeaterTXRXThread::receiveRadio()
{
	...
	for (unsigned int i = 0U; i < length; i++) {
		TRISTATE state = m_demodulator.decode(audio[i] * m_rxLevel, m_rxState == DSRXS_LISTENING);
...
void CDStarRepeaterTXRXThread::transmitNetworkHeader(CHeaderData& header)
{
...
void CDStarRepeaterTXRXThread::transmitNetwork()
{
...
void CDStarRepeaterTXRXThread::transmit()
{
	// Switch off the transmitter?
	if (m_tx) {
		// If no hang time, switch off the TX immediately
...
void CDStarRepeaterTXRXThread::transmitBits(const bool* bits, unsigned int length)
{
	wxASSERT(bits != NULL);
...
void CDStarRepeaterTXRXThread::radioStateMachine(bool bit)
{
	switch (m_rxState) {
...
CHeaderData* CDStarRepeaterTXRXThread::processFECHeader()
{
...
void CDStarRepeaterTXRXThread::processRadioFrame(FRAME_TYPE type)
{
	bool bits[DV_FRAME_LENGTH_BITS];
...
CHeaderData* CDStarRepeaterTXRXThread::processSlowData(bool sync)
{
	bool bits[DV_FRAME_LENGTH_BITS];
	m_bitBuffer.getData(bits, DV_FRAME_LENGTH_BITS);
...
void CDStarRepeaterTXRXThread::callback(const wxFloat32* input, wxFloat32* output, unsigned int nSamples, int id)
{
...
void CDStarRepeaterTXRXThread::readRadioData(const wxFloat32* input, unsigned int nSamples)
{
	wxASSERT(input != NULL);
...
CDStarRepeaterStatusData* CDStarRepeaterTXRXThread::getStatus()
{
	...
	if (m_rptState == DSRS_SHUTDOWN || m_rptState == DSRS_LISTENING)
		return new CDStarRepeaterStatusData(wxEmptyString, wxEmptyString, wxEmptyString, wxEmptyString,
				wxEmptyString, 0x00, 0x00, 0x00, m_tx, m_squelch, m_rxState, m_rptState, 0U, 0U,
				m_activeHangTimer.getTimer(), m_activeHangTimer.getTimeout(), 0U, 0U, 0.0F, noise,
		...
		return new CDStarRepeaterStatusData(m_rxHeader->getMyCall1(), m_rxHeader->getMyCall2(),
				...
				m_rxState, m_rptState, 0U, 0U, m_activeHangTimer.getTimer(), m_activeHangTimer.getTimeout(),
				0U, 0U, (errors * 100.0F) / bits, noise, wxEmptyString, wxEmptyString, wxEmptyString,
...
void CDStarRepeaterTXRXThread::getStatistics()
{
	if (m_rptState == DSRS_VALID)
		m_radioCount++;
...
void CDStarRepeaterTXRXThread::writeStatistics()
{
...
void CDStarRepeaterTXRXThread::blankDTMF(bool* data)
{
	wxASSERT(data != NULL);
...
void CDStarRepeaterTXRXThread::blankDTMF(unsigned char* data)
{
	wxASSERT(data != NULL);
