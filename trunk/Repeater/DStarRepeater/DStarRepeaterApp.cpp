...
void CDStarRepeaterApp::setReader(CWAVFileReader* reader)
{
	return m_thread->setReader(reader);
}

void CDStarRepeaterApp::setRXLevel(wxFloat32 level)
{
	m_thread->setRXLevel(level);
}

void CDStarRepeaterApp::setTXLevel(wxFloat32 level)
{
	m_thread->setTXLevel(level);
}

void CDStarRepeaterApp::setSquelchLevel(wxFloat32 level)
{
...
void CDStarRepeaterApp::getCallsign(wxString& callsign, wxString& gateway, DSTAR_MODE& mode, ACK_TYPE& ack, bool& restriction, bool& rpt1Validation) const
{
	m_config->getCallsign(callsign, gateway, mode, ack, restriction, rpt1Validation);
}

void CDStarRepeaterApp::setCallsign(const wxString& callsign, const wxString& gateway, DSTAR_MODE mode, ACK_TYPE ack, bool restriction, bool rpt1Validation)
{
...
void CDStarRepeaterApp::getRadio(wxString& readDevice, wxString& writeDevice, wxFloat32& rxLevel, wxFloat32& txLevel, SQUELCH_MODE& squelchMode, wxFloat32& squelchLevel, bool& rxInvert, bool& txInvert) const
{
	m_config->getRadio(readDevice, writeDevice, rxLevel, txLevel, squelchMode, squelchLevel, rxInvert, txInvert);
}

void CDStarRepeaterApp::setRadio(const wxString& readDevice, const wxString& writeDevice, wxFloat32 rxLevel, wxFloat32 txLevel, SQUELCH_MODE squelchMode, wxFloat32 squelchLevel, bool rxInvert, bool txInvert)
{
	m_config->setRadio(readDevice, writeDevice, rxLevel, txLevel, squelchMode, squelchLevel, rxInvert, txInvert);
}

void CDStarRepeaterApp::getController(wxString& type, unsigned int& config, int& pttDelay, bool& pttInvert) const
{
	m_config->getController(type, config, pttDelay, pttInvert);
}

void CDStarRepeaterApp::setController(const wxString& type, unsigned int config, int pttDelay, bool pttInvert)
{
	m_config->setController(type, config, pttDelay, pttInvert);
}

void CDStarRepeaterApp::getTimes(unsigned int& timeout, unsigned int& ackTime, unsigned int& hangTime) const
{
	m_config->getTimes(timeout, ackTime, hangTime);
}

void CDStarRepeaterApp::setTimes(unsigned int timeout, unsigned int ackTime, unsigned int hangTime)
{
...
void CDStarRepeaterApp::getActiveHang(unsigned int& time) const
{
	m_config->getActiveHang(time);
}

void CDStarRepeaterApp::setActiveHang(unsigned int time)
{
