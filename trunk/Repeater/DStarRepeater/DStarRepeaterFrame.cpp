...
BEGIN_EVENT_TABLE(CDStarRepeaterFrame, wxFrame)
	EVT_MENU(wxID_OPEN, CDStarRepeaterFrame::onOpen)
...
void CDStarRepeaterFrame::onOpen(wxCommandEvent& event)
{
	...
	if (!res) {
		wxMessageDialog dialog(this, _("Cannot find the file, or the file is corrupt."), _("D-Star Repeater Error"), wxICON_ERROR);
		dialog.ShowModal();
		delete reader;
		return;
	}
	...
	if (reader->getSampleRate() != DSTAR_RADIO_SAMPLE_RATE) {
		...
		wxMessageDialog dialog(this, _("The sound file has an invalid sample rate."), _("D-Star Repeater Error"), wxICON_ERROR);
		dialog.ShowModal();
		reader->close();
		delete reader;
		return;
	...
	if (reader->getChannels() != 1U) {
		...
		wxMessageDialog dialog(this, _("The sound file has too many channels."), _("D-Star Repeater Error"), wxICON_ERROR);
		dialog.ShowModal();
		reader->close();
		delete reader;
		return;
...
void CDStarRepeaterFrame::onAbout(wxCommandEvent& event)
{
	...
	info.SetCopyright(wxT("(C) 2009-2012 using GPL v2 or later"));
...
void CDStarRepeaterFrame::onPreferences(wxCommandEvent& event)
{
	...
	CDStarRepeaterPreferences dialog1(this, -1, callsign, gateway, mode, ack, restriction, rpt1Validation,
