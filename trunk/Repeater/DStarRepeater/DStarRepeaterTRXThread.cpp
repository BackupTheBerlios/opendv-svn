...
void CDStarRepeaterTRXThread::start()
{
	Create();
...
void CDStarRepeaterTRXThread::wait()
{
	Wait();
}

void CDStarRepeaterTRXThread::setReader(CWAVFileReader* reader)
{
...
void CDStarRepeaterTRXThread::setCallsign(const wxString& callsign, const wxString& gateway, DSTAR_MODE mode, ACK_TYPE ack, bool restriction, bool rpt1Validation)
{
...
void CDStarRepeaterTRXThread::setSoundCard(CSoundCardReaderWriter* soundcard, wxFloat32 rxLevel, wxFloat32 txLevel, SQUELCH_MODE squelchMode, wxFloat32 squelchLevel, bool rxInvert, bool txInvert)
{
...
void CDStarRepeaterTRXThread::setTimes(unsigned int timeout, unsigned int ackTime, unsigned int hangTime)
{
...
void CDStarRepeaterTRXThread::setController(CExternalController* controller, int pttDelay)
{
...
void CDStarRepeaterTRXThread::setActiveHang(unsigned int time)
{
...
void CDStarRepeaterTRXThread::setRXLevel(wxFloat32 level)
{
	m_rxLevel = level;
}

void CDStarRepeaterTRXThread::setTXLevel(wxFloat32 level)
{
	m_txLevel = level;
}

void CDStarRepeaterTRXThread::setSquelchLevel(wxFloat32 level)
{
...
void CDStarRepeaterTRXThread::transmitLocal()
{
...
void CDStarRepeaterTRXThread::transmitRadio()
{
...
void CDStarRepeaterTRXThread::transmitNetwork()
{
...
void CDStarRepeaterTRXThread::transmit()
{
	// Switch off the transmitter?
	if (m_tx) {
		// If no hang time, switch off the TX immediately
...
void CDStarRepeaterTRXThread::transmitBits(const bool* bits, unsigned int length)
{
	wxASSERT(bits != NULL);
...
void CDStarRepeaterTRXThread::radioStateMachine(bool bit)
{
	switch (m_rxState) {
...
CHeaderData* CDStarRepeaterTRXThread::processFECHeader()
{
...
void CDStarRepeaterTRXThread::processRadioFrame(FRAME_TYPE type)
{
	bool bits[DV_FRAME_LENGTH_BITS];
...
CHeaderData* CDStarRepeaterTRXThread::processSlowData(bool sync)
{
	bool bits[DV_FRAME_LENGTH_BITS];
	m_bitBuffer.getData(bits, DV_FRAME_LENGTH_BITS);
...
void CDStarRepeaterTRXThread::callback(const wxFloat32* input, wxFloat32* output, unsigned int nSamples, int id)
{
...
void CDStarRepeaterTRXThread::readRadioData(const wxFloat32* input, unsigned int nSamples)
{
	wxASSERT(input != NULL);
...
CDStarRepeaterStatusData* CDStarRepeaterTRXThread::getStatus()
{
	...
	if (m_rptState == DSRS_SHUTDOWN || m_rptState == DSRS_LISTENING) {
		return new CDStarRepeaterStatusData(wxEmptyString, wxEmptyString, wxEmptyString, wxEmptyString, wxEmptyString,
...
void CDStarRepeaterTRXThread::getStatistics()
{
	if (m_rptState == DSRS_VALID)
		m_radioCount++;
...
void CDStarRepeaterTRXThread::writeStatistics()
{
...
void CDStarRepeaterTRXThread::blankDTMF(bool* data)
{
	wxASSERT(data != NULL);
