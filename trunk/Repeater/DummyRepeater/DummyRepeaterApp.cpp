...
const wxString KEY_DVD_DEVICE             = wxT("/dvdDevice");
...
const wxString DEFAULT_DVD_DEVICE         = wxEmptyString;
...
void CDummyRepeaterApp::showMessage(CDummyRepeaterMessageData* message) const
{
...
void CDummyRepeaterApp::getDVDongle(wxString& device) const
{
	wxConfigBase* profile = new wxConfig(APPLICATION_NAME);
	wxASSERT(profile != NULL);
...
void CDummyRepeaterApp::setDVDongle(const wxString& device) const
{
	wxConfigBase* profile = new wxConfig(APPLICATION_NAME);
	wxASSERT(profile != NULL);
