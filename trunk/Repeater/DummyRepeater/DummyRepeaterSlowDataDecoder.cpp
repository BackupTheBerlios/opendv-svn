...
void CDummyRepeaterSlowDataDecoder::addData(const unsigned char* data)
{
	wxASSERT(data != NULL);
...
CDummyRepeaterMessageData* CDummyRepeaterSlowDataDecoder::getMessageData()
{
	if (m_messageData == NULL)
		return NULL;

	CDummyRepeaterMessageData* temp = new CDummyRepeaterMessageData(*m_messageData);
...
void CDummyRepeaterSlowDataDecoder::sync()
{
	m_state = SDD_FIRST;
}

void CDummyRepeaterSlowDataDecoder::reset()
{
	::memset(m_message, 0xFFU, SLOW_DATA_TEXT_LENGTH);
...
void CDummyRepeaterSlowDataDecoder::processMessage()
{
	...
	m_messageData = new CDummyRepeaterMessageData(m_message, SLOW_DATA_TEXT_LENGTH);
}
