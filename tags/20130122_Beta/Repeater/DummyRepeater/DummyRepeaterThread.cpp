...
void CDummyRepeaterThread::setDVDongle(CDVDongleController* dongle)
{
	...
	m_dongle = new CDummyRepeaterDVDongleThread(dongle, &m_decodeAudio, &m_decodeData, &m_encodeAudio, &m_encodeData);
}
...
void CDummyRepeaterThread::processHeader(CHeaderData* header)
{
	...
	m_decodeData.addData(NULL_AMBE_DATA_BYTES, VOICE_FRAME_LENGTH_BYTES);
	m_decodeData.addData(NULL_AMBE_DATA_BYTES, VOICE_FRAME_LENGTH_BYTES);
	m_decodeData.addData(NULL_AMBE_DATA_BYTES, VOICE_FRAME_LENGTH_BYTES);
...
bool CDummyRepeaterThread::processFrame(const unsigned char* buffer, unsigned char seqNo)
{
	...
	while (seqNo != m_networkSeqNo) {
		if (m_networkSeqNo == 0U) {
			...
			CDummyRepeaterMessageData* message = m_slowDataDecoder.getMessageData();
			if (message != NULL)
