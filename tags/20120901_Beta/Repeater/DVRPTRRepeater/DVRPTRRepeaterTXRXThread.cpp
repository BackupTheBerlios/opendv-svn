...
void CDVRPTRRepeaterTXRXThread::start()
{
	Create();
...
void* CDVRPTRRepeaterTXRXThread::Entry()
{
...
void CDVRPTRRepeaterTXRXThread::wait()
{
	Wait();
}

void CDVRPTRRepeaterTXRXThread::setCallsign(const wxString& callsign, const wxString& gateway, DSTAR_MODE mode, ACK_TYPE ack, bool restriction, bool rpt1Validation)
{
...
void CDVRPTRRepeaterTXRXThread::setModem(CDVRPTRController* controller)
{
...
void CDVRPTRRepeaterTXRXThread::setWhiteList(CCallsignList* list)
{
}
...
CDVRPTRRepeaterStatusData* CDVRPTRRepeaterTXRXThread::getStatus()
{
	...
	if (m_rptState == DSRS_SHUTDOWN || m_rptState == DSRS_LISTENING)
		return new CDVRPTRRepeaterStatusData(wxEmptyString, wxEmptyString, wxEmptyString, wxEmptyString,
				wxEmptyString, 0x00, 0x00, 0x00, m_tx, m_rxState, m_rptState, 0U, 0U, 0U, 0U, 0.0F,
		...
		return new CDVRPTRRepeaterStatusData(m_rxHeader->getMyCall1(), m_rxHeader->getMyCall2(),
				...
				m_rptState, 0U, 0U, 0U, 0U, (errors * 100.0F) / bits, wxEmptyString, wxEmptyString,
...
void CDVRPTRRepeaterTXRXThread::blankDTMF(unsigned char* data)
{
	wxASSERT(data != NULL);
