...
#include "DummyRepeaterBleeper.h"
...
CDummyRepeaterDVDongleThread::CDummyRepeaterDVDongleThread(CDVDongleController* dongle, CRingBuffer<wxFloat32>* decodeAudio, CRingBuffer<unsigned char>* decodeData, CRingBuffer<wxFloat32>* encodeAudio, CRingBuffer<unsigned char>* encodeData) :
...
CDummyRepeaterDVDongleThread::~CDummyRepeaterDVDongleThread()
{
}

void* CDummyRepeaterDVDongleThread::Entry()
{
	m_dongle->start();
...
void CDummyRepeaterDVDongleThread::kill()
{
	m_killed = true;
}

void CDummyRepeaterDVDongleThread::setMode(DVDMODE mode)
{
...
void CDummyRepeaterDVDongleThread::setBleep(bool on)
{
	m_bleep = on;
}

void CDummyRepeaterDVDongleThread::processDecode()
{
...
void CDummyRepeaterDVDongleThread::processEncode()
{
...
void CDummyRepeaterDVDongleThread::processIdle()
{
	unsigned char ambe[VOICE_FRAME_LENGTH_BYTES];
	::memset(ambe, 0x00, VOICE_FRAME_LENGTH_BYTES);
...
void CDummyRepeaterDVDongleThread::sendBleep()
{
	wxFloat32 audio[DSTAR_RADIO_BLOCK_SIZE];
	unsigned int n;

	CDummyRepeaterBleeper bleep(DSTAR_RADIO_SAMPLE_RATE, DSTAR_BLEEP_FREQ, DSTAR_BLEEP_LENGTH, DSTAR_BLEEP_AMPL);
...
void CDummyRepeaterDVDongleThread::upSample(const wxFloat32* audioIn, wxFloat32* audioOut)
{
	unsigned int out = 0U;
...
void CDummyRepeaterDVDongleThread::downSample(const wxFloat32* audioIn, wxFloat32* audioOut)
{
	unsigned int in = 0U;
