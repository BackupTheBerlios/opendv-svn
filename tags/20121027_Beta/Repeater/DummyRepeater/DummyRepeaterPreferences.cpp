...
CDummyRepeaterPreferences::CDummyRepeaterPreferences(wxWindow* parent, int id, const wxString& callsign1,
								   ...
								   const wxString& writeDevice, const wxString& dvdDevice,
...
m_bleep(NULL)
{
	...
	m_dongle = new CDummyRepeaterDVDongleSet(noteBook, -1, APPLICATION_NAME, dvdDevice);
	noteBook->AddPage(m_dongle, _("DV-Dongle"), false);

	m_network = new CNetworkSet(noteBook, -1, APPLICATION_NAME, gwyAddress, gwyPort, localAddress, localPort);
...
wxString CDummyRepeaterPreferences::getDVDDevice() const
{
