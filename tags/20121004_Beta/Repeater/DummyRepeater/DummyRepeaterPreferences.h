...
#include "DummyRepeaterDVDongleSet.h"
...
public:
	CDummyRepeaterPreferences(wxWindow* parent, int id, const wxString& callsign1, const wxString& callsign2,
		const wxString& readDevice, const wxString& writeDevice, const wxString& dvdDevice,
		const wxString& gwyAddress, unsigned int gwyPort, const wxString& localAddress, unsigned int localPort,
	    const wxString& type, unsigned int config, bool pttInvert, bool squelchInvert, unsigned int timeout,
...
private:
	...
	CDummyRepeaterDVDongleSet*   m_dongle;
