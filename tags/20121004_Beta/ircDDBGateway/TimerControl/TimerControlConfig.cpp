...
void CTimerControlConfig::setGateway(const wxString& address, unsigned int port, const wxString& password) const
{
	wxString addressKey  = m_name + KEY_ADDRESS;
	wxString portKey     = m_name + KEY_PORT;
	wxString passwordKey = m_name + KEY_PASSWORD;
...
{
	wxString delayKey = m_name + KEY_DELAY;

	m_config->Read(delayKey, &delay, DEFAULT_DELAY);
}

void CTimerControlConfig::setDelay(bool delay) const
{
	wxString delayKey = m_name + KEY_DELAY;

	m_config->Write(delayKey, delay);
	m_config->Flush();
}
