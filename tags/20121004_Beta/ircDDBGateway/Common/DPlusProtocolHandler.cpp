...
CHeaderData* CDPlusProtocolHandler::readHeader()
{
	...
	bool res = header->setDPlusData(m_buffer, m_length, false, m_address, m_port);
	if (!res) {
		delete header;
		return NULL;
	}
...
CAMBEData* CDPlusProtocolHandler::readAMBE()
{
	...
	bool res = data->setDPlusData(m_buffer, m_length, m_address, m_port);
	if (!res) {
		delete data;
		return NULL;
	}
...
CConnectData* CDPlusProtocolHandler::readConnect()
{
	...
	bool res = connect->setDPlusData(m_buffer, m_length, m_address, m_port);
	if (!res) {
		delete connect;
		return NULL;
	}
